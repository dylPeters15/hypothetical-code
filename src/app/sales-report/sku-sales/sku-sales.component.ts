...
export class SkuSalesComponent implements OnInit, ControlValueAccessor {
  ...
  async refreshData() {
    ...
    this.allSales = await this.restv2.getSales(AndVsOr.AND, this.sku['_id'], this.selectedCustomerId=="all"?null:this.selectedCustomerId, new Date(new Date().getFullYear()-10, new Date().getMonth()), null, 54321);
