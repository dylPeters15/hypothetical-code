...
export class NewSkuDialogComponent implements OnInit {
  ...
  createSku() {
    ...
    if (this.edit == false)
    {
      ...
      this.rest.adminCreateSku(this.name, this.sku_number, this.case_upc_number, this.unit_upc_number, this.unit_size, this.count_per_case, this.product_line, this.ingredients, this.comment, id).subscribe(response => {
        let i;
